...
\item dataCounter = 0
\end{itemize}
\item getData(src url):
\begin{enumerate}
\item nummer=dataCounter
\item dataCounter ++
\item Hole Daten
\item .then(setData(nummer, Daten))
\item starte Übergang zu neutraler/leerer Darstellung
\end{enumerate}
...
\begin{itemize}
\item zeige ``<komponente> am Laden'' an.
\item setData(leeres Gerüst, Tabelle, Koordinatensystem)
